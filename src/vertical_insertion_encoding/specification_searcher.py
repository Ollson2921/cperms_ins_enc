...
class ConfigAvoidingBasis(CombinatorialClass[CPermutation]):
    ...
    def __init__(self, config: Configuration, basis: List[CayleyPermutation]) -> None:
        self.config = config
        self.basis = basis
        super().__init__()
...
class DeleteIndexStrategy(CartesianProductStrategy[ConfigAvoidingBasis, CPermutation]):
...
pack = StrategyPack(
    initial_strats=[IndexDeletingFactory()],
    inferral_strats=[],
    expansion_strats=[[ApplyLetterStrategy()]],
    ver_strats=[AtomStrategy()],
    name="Finding regular grammar for Cayley Permutations.",
)
