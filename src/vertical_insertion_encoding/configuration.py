...
class Letter:
    ...
    def apply(self, config: "Configuration") -> "Configuration":
        ...
        Examples:
        >>> print(Letter("l", 1, 0).apply(Configuration([0, 1, "🔹", 2])))
        012🔹2
        >>> print(Letter("f", 1, 1).apply(Configuration([0, 1, "🔹", 2])))
        0132
        """
        if self.letter == "m":
    ...
    def apply_m(
        self, config: "Configuration", index: int, repeat: int
    ) -> "Configuration":
...
class Configuration:
    """
    A configuration is a list where the values are integers or the
    slot represented by "🔹".

    Examples:
    >>> print(Configuration([0, 1, "🔹", 2]))
    01🔹2
    """
    ...
    def __init__(self, config: List):
        ...
        for elm in self.config:
            if not isinstance(elm, int) and elm != "🔹":
                raise ValueError("Configuration must be a list of integers or '🔹'.")
        ...
        if len(self.config) > 1:
            ...
            for val in range(max(values)):
                if val not in values:
                    raise ValueError(
                        "Values in configuration must be consecutive and zero based."
                    )

    def apply_m(self, index: int, repeat: int) -> "Configuration":
        """
        Change "🔹" to: "🔹", max_value + repeat, "🔹".

        Example:
        >>> print(Configuration([0, 1, "🔹", 2]).apply_m(1, 1))
        01🔹3🔹2
        """
    ...
    def apply_l(self, index: int, repeat: int) -> "Configuration":
        """
        Change "🔹" to: max_value + repeat, "🔹".

        Example:
        >>> print(Configuration([0, 1, "🔹", 2]).apply_l(1, 1))
        013🔹2
        """
        idx = self.find_slot_index(index)
        max_value = self.max_value() + repeat
    ...
    def apply_r(self, index: int, repeat: int) -> "Configuration":
        """
        Change "🔹" to: "🔹", max_value + repeat.

        Example:
        >>> print(Configuration([0, 1, "🔹", 2]).apply_r(1, 1))
        01🔹32
        """
        idx = self.find_slot_index(index)
        max_value = self.max_value() + repeat
    ...
    def apply_f(self, index: int, repeat: int) -> "Configuration":
        """
        Change "🔹" to: max_value + repeat.

        Example:
        >>> print(Configuration([0, 1, "🔹", 2]).apply_f(1, 1))
        0132
        """
        idx = self.find_slot_index(index)
        max_value = self.max_value() + repeat
    ...
    def find_slot_index(self, index: int) -> int:
        """
        Returns the index in the configuration as a list of
        the nth slot where n is the input 'index'.

        Examples:
        >>> print(Configuration([0, "🔹", 1, "🔹", 2]).find_slot_index(1))
        1
        >>> print(Configuration([0, "🔹", 1, "🔹", 2]).find_slot_index(2))
        3
        >>> print(Configuration([0, "🔹", 1]).find_slot_index(1))
        1
        """
        count = 0
    ...
    def max_value(self) -> int:
        """Returns the largest value in the configuration, ignoring slots.
        If the configuration is a single slot, returns 0

        Examples:
        >>> Configuration([0, 1, "🔹", 2]).max_value()
        2
        """
        if self.config == ["🔹"]:
            return -1
    ...
    def has_increased(self) -> bool:
        ...
        (True if there is more than one of the maximum value in the configuration)

        Examples:
        >>> print(Configuration([0, 1, "🔹", 2]).has_increased())
        True
        >>> print(Configuration([0, 2, 1, "🔹", 2]).has_increased())
        False
        """
        count = 0
        for elm in self.config:
    ...
    def letter_of_last_insertion(self) -> Letter:
        ...
        Example:
        >>> print(Configuration([0, 2, 1, "🔹", 2]).letter_of_last_insertion())
        f_(1, 0)
        """
    ...
    def counting_slots(self, index: int) -> int:
        ...
        Examples:
        >>> Configuration([0, "🔹", 2, 1, "🔹", 2]).counting_slots(2)
        1
        >>> Configuration([0, "🔹", 2, 1, "🔹", 2]).counting_slots(5)
        2
        """
        count = 0
        for idx, val in enumerate(self.config):
    ...
    def undoing_last_ins(self, letter: Letter) -> "Configuration":
        """Returns the configuration before the last letter was applied.

        Example:
        >>> print(Configuration([0, 2, 1, "🔹", 2]).undoing_last_ins(Letter("f", 1, 0)))
        0🔹1🔹2
        """
        index = self.index_of_max()
        if letter.letter == "m":
    ...
    def get_word(self) -> "Word":
        ...
        Example:
        >>> print(Configuration([0, 2, 1, "🔹", 2]).get_word())
        l_(1, 1)m_(1, 1)r_(2, 1)f_(1, 0)
        """
        configuration = self
        word: List[Letter] = []
        while configuration != self.__class__(["🔹"]):
            letter = configuration.letter_of_last_insertion()
            word = [letter] + word
            configuration = configuration.undoing_last_ins(letter)
        return Word(word)

    @classmethod
    def standardise(cls, config: List) -> "Configuration":
        """Standardises a configuration by replacing the numbers with the
        smallest possible numbers that give the same relative sequence.

        Example:
        >>> print(Configuration.standardise([2, "🔹", 3]))
        0🔹1
        """
    ...
    def delete_index(self, index: int) -> "Configuration":
        """Removing the element at index 'index' from the configuration and standardise

        Examples:
        >>> print(Configuration([0, 1, "🔹", 2]).delete_index(1))
        0🔹1
        >>> print(Configuration([0, 1, "🔹", 2]).delete_index(3))
        01🔹
        """
    ...
    def candidates_to_delete(self) -> List[int]:
        ...
        Example:
        >>> Configuration([0, 1, "🔹", 2,"🔹", 1]).candidates_to_delete()
        [0, 1, 5]
        """
        candidates = []
    ...
    def deleteable_indices(self, basis: List[CayleyPermutation]) -> List[int]:
        ...
        configuration still avoids the basis after being deleted."""
    ...
    def cperm_idx_from_config_idx(
        ...
        index of the configuration it was obtained from

        Examples:
        >>> Configuration([0, "🔹", 1, 2]).cperm_idx_from_config_idx(
    ...
    def cayley_perms(
        ...
        Example:
        >>> for config in Configuration([0, 1, "🔹"]).cayley_perms(4, [CayleyPermutation([1, 0])]):
        ...     print(config)
        012
        0123
        0122
    ...
    def count(self, size: int, basis: List[CayleyPermutation]):
        ...
        up to length 'size' which avoid the basis from the configuration.

        Example:
        >>> Configuration([0, 1, "🔹"]).count(4, [CayleyPermutation([1, 0])])
        0,0,0,1,2,
        """
    ...
    def number_of_slots(self) -> int:
        """Returns the number of slots in the configuration.

        Example:
        >>> Configuration([0, "🔹", 1, "🔹", 2]).number_of_slots()
        2
        """
        count = 0
    ...
    def number_of_numbers(self) -> int:
        """Returns the number of numbers in the configuration
        (total length minus number of slots)."""
        return len(self.config) - self.number_of_slots()

    def all_possible_letters(self) -> List[Letter]:
        """Returns a list of all possible letters that can be inserted into the configuration.

        Examples:
        >>> for letter in Configuration([0, 1, "🔹", 2]).all_possible_letters():
        ...     print(letter)
        f_(1, 1)
        l_(1, 1)
        m_(1, 1)
    ...
    def _new_max_letters(self, left: int, right: int) -> List[Letter]:
        ...
        configuration with a new maximum value."""
    ...
    def children(self) -> List["Configuration"]:
        """Returns a list of all configurations that can be obtained
        by applying one letter to the starting configuration.

        Examples:
        >>> for config in Configuration([0, 1, "🔹", 2]).children():
        ...     print(config)
        0122
        012🔹2
        01🔹2🔹2
        ...
        01🔹3🔹2
        01🔹32
        >>> for config in Configuration(["🔹"]).children():
        ...     print(config)
        0
        0🔹
        🔹0🔹
    ...
    def avoids_basis(self, basis: List[CayleyPermutation]) -> bool:
        """Returns True if the values in the configuration avoids the basis.

        Examples:
        >>> Configuration([0, 1, 2, "🔹", 2]).avoids_basis([CayleyPermutation([1, 0])])
        True
        >>> Configuration([0, 1, 3, "🔹", 2]).avoids_basis([CayleyPermutation([1, 0])])
        False
        """
        new_config = []
        for val in self.config:
    ...
    def is_cayley_perm(self) -> bool:
        """Returns True if the configuration is a Cayley permutation (has no slots)."""
        for val in self.config:
            if val == "🔹":
                return False
        return True
...
class Word:
    ...
    def words_size_n(cls, av: Av, size: int) -> Iterator["Word"]:
        ...
        for cperm in av.generate_cperms(size):
            config = Configuration(cperm.cperm)
